 ...
 */
	...
	if (pending_path==NULL) {
		...
		if (pending_path==NULL) {
			(void)fprintf(stderr, "%s malloc(%lu) for pending path of bufr->dbnet failed",
				myname, (unsigned long) dbn_root_plen+strlen(TMP_QUE_NAME)+FN_LEN);
			return 0;
		}
...
static int WriteToStream(char *subtype, unsigned char *msg, size_t mlen) {
	...
	if (fwrite(msg, msg_element_sz, mlen, fs) != mlen) {
		(void)fprintf(stderr, "%s fwrite(%s) failed - mlen=%lu", myname, pending_path, (unsigned long) mlen);
		return 0;
	}
