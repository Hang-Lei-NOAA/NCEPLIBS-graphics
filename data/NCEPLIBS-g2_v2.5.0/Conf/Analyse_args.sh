 if (( $# == 0 )); then
   echo "*** Usage: $0 dell|cray|phase1/2|theia|hera|gaea|jet|intel_general|gnu_general libver=<lib_ver> [debug] [compileonly] [prefix=<installpath>]" >&2
   exit 1
 fi
 sys=${1,,}
 [[ $sys == dell || $sys == cray || $sys == phase1/2 || \
